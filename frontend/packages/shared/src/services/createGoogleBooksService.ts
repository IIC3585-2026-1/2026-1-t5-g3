...
export function createGoogleBooksService(apiKey: string | null | undefined) {
  function getApiKey(): string | null {
    if (!apiKey) {
      console.error('Falta la variable de entorno VITE_GOOGLE_BOOKS_API_KEY')
      return null
    }
    return apiKey
  }
  ...
  return {
    ...
    async getBookDetail(volumeId: string): Promise<BookDetail | null> {
      const key = getApiKey()
      if (!key) return null
