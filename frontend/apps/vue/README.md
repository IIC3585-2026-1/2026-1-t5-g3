...
- Node.js 24+ (desde `frontend/.nvmrc`)
- [pnpm](https://pnpm.io/) desde la carpeta `frontend/`

## Configuración

La API key se lee desde el `.env` en la **raíz del repositorio** (`2026-1-t5-g3/.env`).
...
2. Edita el `.env` en la raíz del repo y agrega tus variables:
...
Solo esta app: `pnpm dev` → `http://localhost:5173`
...
La otra implementación está en `frontend/apps/svelte/`. Ambas usan `@mybooks/shared`. Para detalles históricos sobre la conversión
